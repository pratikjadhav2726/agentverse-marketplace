...
if (process.env.NODE_ENV === 'production') {
  ...
  sqlite = new Database('agentverse.db');
...
export async function initializeDatabase() {
  if (process.env.NODE_ENV === 'production') {
    // For production, we assume the schema is already set up in Supabase
    console.log('Production database schema should be managed via Supabase migrations');
    return;
  }

  // Development: Initialize SQLite schema
  if (!sqlite) {
